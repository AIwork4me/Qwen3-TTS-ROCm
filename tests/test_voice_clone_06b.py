# tests/test_voice_clone_06b.py
"""Task 1A: GPU integration -- voice clone workflow suite (real 0.6B Base weights).

Mirror of ``tests/test_voice_clone_workflow.py`` (the 1.7B Base suite) for
alias ``base-0.6b``: exercises the FULL clone surface of the official 0.6B
Base model on the Radeon 8060S (gfx1151), loaded via
:func:`qwen3_tts_rocm.loader.load`: direct (ref_audio, ref_text) cloning,
x-vector-only cloning, the reusable ``create_voice_clone_prompt`` ->
``generate_voice_clone(voice_clone_prompt=...)`` path, save/load PARITY with
the OFFICIAL demo payload format, and batched cloning.  Exploratory-integration
TDD: genuine failures are product bugs and get debugged systematically --
...
x_vector_only_mode, voice_clone_prompt, non_streaming_mode, ...)`` (ground
truth is the installed package); keyword names are identical to the 1.7B
suite, so every call below uses keywords only.

Latency guardrail (binding program constraint, mirrors the 1.7B suite): every
...
same as the 1.7B suite): clone voice quality on synthetic babble is not
perceptually representative of a real human reference; the correctness gate
here is official-API sanity (non-silent finite float waveforms at 24 kHz), so
REF_TEXT is an approximate transcript rather than an exact one.
...
roundtrip test replays both sides verbatim and additionally asserts schema
equality against the REAL official dataclass fields.

Robustness policy (binding, same as the 1.7B suites): generation is
stochastic; every test's primary net is
:func:`qwen3_tts_rocm.testing.assert_wav_sane`; distinction assertions are
robust-only via conftest ``_distinct``.
"""
...
#: (same clip as the 1.7B suite; see module docstring limitation note).
...
#: suite (binding program constraint; mirrors the 1.7B suite).
MAX_NEW_TOKENS = 512
...
def bc06_model(gpu):
    """Load the 0.6B Base model once for this module; unload on teardown."""
    m = loader.load("base-0.6b")
    yield m
    loader.unload(m)


@pytest.fixture(scope="module")
def bc06_prompt_items(bc06_model):
    """Reusable clone prompt built ONCE per module (encode + speaker embed)."""
    t0 = time.perf_counter()
    items = bc06_model.create_voice_clone_prompt(
...
def test_clone_with_ref_text(bc06_model):
...
def test_clone_x_vector_only(bc06_model):
...
def test_create_prompt_reuse(bc06_model, bc06_prompt_items):
    ...
    (exactly as the 1.7B suite does): ``icl_mode`` flips with
    ``x_vector_only_mode``, ICL carries ref_code + speaker embedding + the
    reference transcript forward.
    """
    from qwen_tts import VoiceClonePromptItem

    items = bc06_prompt_items
    assert len(items) == 1
    item = items[0]
    ...
    first, sr = _timed_generate(
        bc06_model, "generate_voice_clone", text=TEXT, language="Auto",
    ...
    second, _ = _timed_generate(
        bc06_model, "generate_voice_clone", text=ALT_TEXT, language="Auto",
        voice_clone_prompt=items, max_new_tokens=MAX_NEW_TOKENS,
    )
...
def test_save_load_roundtrip_parity(bc06_model, bc06_prompt_items, tmp_path):
...
def test_batch_clone(bc06_model):
